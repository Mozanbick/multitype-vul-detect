...
def get_stats(array,
    ...
    Args:
        array (array like object): The numerical array, this array can be convert to `torch.Tensor`
        conf_interval (bool, optional): If True, compute the confidence interval bound (95%)
            instead of the std value. (default: False)
        name (str, optional): The name of this numerical array, for log usage. (default: None)
        stdout (bool, optional): Whether to output result to terminal. (default: False)
        logout (bool, optional): Whether to output result via logging module. (default: False)
    """
    eps = 1e-9
...
def get_batch_id(num_nodes: torch.Tensor):
    """
    Convert the num_nodes array obtained from batch graph to batch_id array for each node

    Args:
        num_nodes (torch.Tensor): The tensor whose element is the number of nodes in each
            graph in the batch graph
    """
...
def topk(x: torch.Tensor,
    ...
    """
    The top-k pooling method. Given a graph batch, this method will pool out some nodes from
    input node feature tensor for each graph according to the given ratio.

    Args:
        x (torch.Tensor): The input node feature batch-tensor to be pooled.
        ratio (float): The pool ratio. If `ratio = 0.5` then half of the input tensor will be pooled out.
